...
本示例 `--metric http_user_requests_total` → `doctor_report.json`（`match_count: 1`）：字段含 `query`
（用了哪些选择器）、`report`（scan 元信息）、`matches`（命中的无效指标详情）、`notes`。
...
prom-ai-guard doctor --report reports/analysis_report.json --metric http_user_requests_total --json examples/doctor/doctor_report.json
...
This example, `--metric http_user_requests_total` → `doctor_report.json` (`match_count: 1`): fields
include `query` (which selectors were used), `report` (scan metadata), `matches` (the matched invalid
metrics), and `notes`.
